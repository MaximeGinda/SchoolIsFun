...
public class CourseActivity extends AppCompatActivity implements PopupMenu.OnMenuItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        if(HomeActivity.isConnected) {
            ...
            if (database.courseContentDao().findboolSummaryWithID(getIntent().getIntExtra("id", 0),nameClasse )) {
                checked1.setImageResource(R.drawable.checked);
            }
            if (database.courseContentDao().findboolVideoWithID(getIntent().getIntExtra("id", 0),nameClasse)) {
                checked2.setImageResource(R.drawable.checked);
            }
            if (database.courseContentDao().findboolExerciseWithID(getIntent().getIntExtra("id", 0),nameClasse)) {
                checked3.setImageResource(R.drawable.checked);
            }
            if (database.courseContentDao().findboolQuizWithID(getIntent().getIntExtra("id", 0), nameClasse)) {
